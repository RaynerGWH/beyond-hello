...
function AIGameplay() {
  ...
  const scenario = state?.generatedScenario || null;
  const scenarioId = scenario?.id || null;
  const customTitle = scenario?.title || state?.scenarioTitle || 'AI Conversation';
  ...
  // Accumulate results across scenes
  ...
  useEffect(() => {
    ...
    setCountdown(null);
  }, [currentScene, navigate, scenario]);
  ...
  const advanceScene = (results) => {
    ...
    if (nextIndex >= totalScenes) {
      navigate('/feedback', {
        state: { scenarioId, sceneResults: results, generatedScenario: scenario }
      });
    } else {
  ...
  // -- Guard -----------------------------------------------------------------
  if (!scenario || !currentScene) return null;
  ...
  return (
    <div className="gameplay-page">
      ...
      <button className="gameplay-home-btn" onClick={() => navigate('/hub')} aria-label="Back to Scenarios">
      ...
      <div className="gameplay-header">
        <h1 className="gameplay-title">{customTitle}</h1>
